...
**Upstream closure**: 195 packages. **U2 installs**: 99. **Browser plugins**: 36 of 41.
...
| Client plugin graph | ours | 36 of 41 browser plugins staged and listed, plus the boot prelude U1 injects. Since rc.8 that prelude is generated by upstream's own `injectBootManifest` rather than written here: the shell no longer carries the module loader, and a hand-written copy of the protocol is what took the page down once |
